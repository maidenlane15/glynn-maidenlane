...
exports.handler = async function (event) {
  ...
  if (body.action === 'find') {
    try {
      var q = String(body.search || '').slice(0, 80);
      var gen = String(body.gender || '').toLowerCase();
      var url = 'https://api.elevenlabs.io/v1/shared-voices?page_size=12&language=en' + (q ? '&search=' + encodeURIComponent(q) : '') + ((gen === 'male' || gen === 'female') ? '&gender=' + gen : '');
      var rf = await fetch(url, { headers: { 'xi-api-key': key } });
      var df = await rf.json();
      var vs = (df && df.voices) || [];
      if (!vs.length) return { statusCode: 200, headers: HDR, body: JSON.stringify({ error: 'No library voices matched' }) };
      vs.sort(function (a, b) { return (b.cloned_by_count || 0) - (a.cloned_by_count || 0); });
      var pick = vs[0];
      var nm2 = String(body.name || 'Glynn Voice').slice(0, 60);
      var ra = await fetch('https://api.elevenlabs.io/v1/voices/add/' + encodeURIComponent(pick.public_owner_id) + '/' + encodeURIComponent(pick.voice_id), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'xi-api-key': key },
        body: JSON.stringify({ new_name: nm2 })
      });
      var da = await ra.json();
      if (!ra.ok || !da.voice_id) {
        var ma = (da && da.detail && (da.detail.message || da.detail)) || 'Could not add library voice';
        return { statusCode: 200, headers: HDR, body: JSON.stringify({ error: String(typeof ma === 'string' ? ma : JSON.stringify(ma)).slice(0, 300) }) };
      }
      return { statusCode: 200, headers: HDR, body: JSON.stringify({ voice_id: da.voice_id, picked: pick.name || '' }) };
    } catch (e2) {
      return { statusCode: 200, headers: HDR, body: JSON.stringify({ error: String(e2 && e2.message ? e2.message : e2) }) };
    }
  }
