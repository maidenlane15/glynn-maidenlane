...
exports.handler = async function(event) {
  ...
  if (action === 'clone') {
    try {
      ...
      // B3.29: clone from up to 5 samples at once (multiple files = truer voice)
      const uris = (Array.isArray(body.audios) && body.audios.length) ? body.audios.slice(0, 5) : (body.audio ? [body.audio] : []);
      if (!uris.length) return { statusCode: 200, body: JSON.stringify({ error: 'No audio provided for cloning.' }) };
      ...
      for (let i = 0; i < uris.length; i++) {
        const m = /^data:([^;]+);base64,(.*)$/.exec(uris[i] || '');
        if (!m) continue;
        const mime = m[1];
        const bytes = Buffer.from(m[2], 'base64');
        const ext = /mp4|m4a|aac/i.test(mime) ? 'm4a' : /mpeg|mp3/i.test(mime) ? 'mp3' : /wav/i.test(mime) ? 'wav' : /ogg|webm/i.test(mime) ? 'webm' : 'm4a';
        fd.append('files', new Blob([bytes], { type: mime }), 'sample' + (i + 1) + '.' + ext);
        added++;
      }
      if (!added) return { statusCode: 200, body: JSON.stringify({ error: 'Audio samples could not be read.' }) };
    ...
    try {
      ...
      const r = await fetch('https://api.elevenlabs.io/v1/text-to-speech/' + encodeURIComponent(vid), {
        ...
        body: JSON.stringify({
          ...
          seed: (Number.isInteger(body.seed) && body.seed >= 0 && body.seed <= 4294967295) ? body.seed : undefined,
          voice_settings: (function (vs) {
            // B3.21: per-brand tuning passthrough from the app; clamps to ElevenLabs' 0..1 range
            var c = function (x, d) { x = Number(x); return (isFinite(x) && x >= 0 && x <= 1) ? x : d; };
            vs = vs || {};
            var o = {
              stability: c(vs.stability, 0.5),
              similarity_boost: c(vs.similarity_boost, 0.8),
              style: c(vs.style, 0.0),
              use_speaker_boost: (typeof vs.use_speaker_boost === 'boolean') ? vs.use_speaker_boost : true
            };
            var sp = Number(vs.speed);
            if (isFinite(sp) && sp >= 0.7 && sp <= 1.2 && sp !== 1) o.speed = sp;
            return o;
          })(body.voice_settings)
        })
      });
  ...
  if (action === 'voices') {
    try {
      ...
      if (!r.ok) { let et = ''; try { et = await r.text(); } catch (e) {} return { statusCode: 200, body: JSON.stringify({ error: 'Voices HTTP ' + r.status + ': ' + et.slice(0, 180) }) }; }
      const j = await r.json();
      const vs = (j.voices || []).map(function (v) { return { voice_id: v.voice_id, name: v.name, category: v.category || '' }; });
      return { statusCode: 200, body: JSON.stringify({ voices: vs }) };
    } catch (e) { return { statusCode: 200, body: JSON.stringify({ error: 'Voices error: ' + e.message }) }; }
  }
