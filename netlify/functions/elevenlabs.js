...
exports.handler = async function(event) {
  ...
  if (action === 'verify') {
    try {
      ...
      if (r.status === 401) return { statusCode: 200, body: JSON.stringify({ error: 'Key rejected (401). It is wrong, revoked, or has extra characters. Regenerate at elevenlabs.io and paste the new one.' }) };
      let et = ''; try { et = await r.text(); } catch (e) {}
      return { statusCode: 200, body: JSON.stringify({ error: 'Key check HTTP ' + r.status + ': ' + et.slice(0, 140) }) };
    } catch (e) {
      return { statusCode: 200, body: JSON.stringify({ error: 'Key check error: ' + e.message }) };
    }
  }
