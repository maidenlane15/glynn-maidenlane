...
exports.handler = async function(event) {
  ...
  // ── FETCH VIDEO (same-origin proxy so the browser can export with a new voice) ──
  if (action === 'fetch_video') {
    // B3.32: chunked proxy \u2014 any video size, ~3MB slices, safely under Netlify's response cap.
    const vurl = (typeof body.video_url === 'string' && /^https:\/\//.test(body.video_url)) ? body.video_url : null;
    if (!vurl) return { statusCode: 400, body: JSON.stringify({ error: 'video_url required' }) };
    const CHUNK = 3 * 1024 * 1024;
    const offset = (Number.isInteger(body.offset) && body.offset >= 0) ? body.offset : 0;
    try {
      const r = await fetch(vurl, { headers: { Range: 'bytes=' + offset + '-' + (offset + CHUNK - 1) } });
      if (!r.ok && r.status !== 206) {
        return { statusCode: 200, body: JSON.stringify({ error: 'Video fetch failed: HTTP ' + r.status }) };
      }
      let buf = Buffer.from(await r.arrayBuffer());
      let total = null;
      if (r.status === 206) {
        const cr = r.headers.get('content-range');
        const mm = cr ? /\/(\d+)\s*$/.exec(cr) : null;
        if (mm) total = parseInt(mm[1], 10);
      } else {
        total = buf.length;                       // server ignored Range: we got the whole file
        buf = buf.slice(offset, offset + CHUNK);
      }
      if (!buf.length) return { statusCode: 200, body: JSON.stringify({ error: 'Empty chunk at offset ' + offset }) };
      const done = (total !== null) ? (offset + buf.length >= total) : (buf.length < CHUNK);
      const mime = r.headers.get('content-type') || 'video/mp4';
      return { statusCode: 200, body: JSON.stringify({ b64: buf.toString('base64'), mime: mime, total: total, next: offset + buf.length, done: done }) };
    } catch (e) {
      return { statusCode: 200, body: JSON.stringify({ error: 'Video proxy error: ' + e.message }) };
    }
  }
