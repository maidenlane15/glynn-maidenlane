...
exports.handler = async function(event) {
  ...
  if (action === 'fetch_video') {
    ...
    try {
      ...
      if (!r.ok) return { statusCode: 200, body: JSON.stringify({ error: 'Video fetch failed: HTTP ' + r.status }) };
      const buf = Buffer.from(await r.arrayBuffer());
      if (buf.length > 5.5 * 1024 * 1024) {
        return { statusCode: 200, body: JSON.stringify({ error: 'Video too large to export in-browser (' + (buf.length / 1048576).toFixed(1) + 'MB) \u2014 use Redub for the final file.' }) };
      }
      const mime = r.headers.get('content-type') || 'video/mp4';
      return { statusCode: 200, body: JSON.stringify({ b64: buf.toString('base64'), mime: mime }) };
    } catch (e) {
