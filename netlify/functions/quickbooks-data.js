...
// Netlify's automatic Blobs context detection has proven unreliable for this
// function in testing (confirmed MissingBlobsEnvironmentError even with
// getStore() called correctly inside the handler). Falling back to explicit
// siteID/token — Netlify's own documented, guaranteed-to-work alternative —
// whenever those are available as environment variables.
...
exports.handler = async function (event) {
  ...
  const store = getBlobsStore('quickbooks');
  let record;
  ...
  // Run all four independent sections in parallel — total time is roughly
  // the slowest single call instead of the sum of all four, which is what
  // was pushing this function past Netlify's execution time limit before.
  async function fetchProfitLoss() {
    ...
    return { plIncome: plIncome, plExpense: plExpense };
  }
  ...
  async function fetchOpenInvoices() {
    ...
    return { openInvoicesTotal: openInvoicesTotal, openInvoicesCount: openInvoicesCount };
  }
  ...
  async function fetchOpenBills() {
    ...
    return { openBillsTotal: openBillsTotal, openBillsCount: openBillsCount };
  }
  ...
  async function fetchCounts() {
    ...
    return { customerCount: customerCount, vendorCount: vendorCount };
  }

  const [pl, inv, bills, counts] = await Promise.all([
    fetchProfitLoss(),
    fetchOpenInvoices(),
    fetchOpenBills(),
    fetchCounts()
  ...
  return {
    ...
    body: JSON.stringify({
      ...
      profitLoss: { mtdIncome: pl.plIncome, mtdExpense: pl.plExpense, mtdNet: pl.plIncome - pl.plExpense },
      openInvoicesTotal: inv.openInvoicesTotal, openInvoicesCount: inv.openInvoicesCount,
      openBillsTotal: bills.openBillsTotal, openBillsCount: bills.openBillsCount,
      customerCount: counts.customerCount, vendorCount: counts.vendorCount
    })
  };
};
