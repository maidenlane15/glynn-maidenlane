...
exports.handler = async function(event) {
  ...
  try {
    const { runway_key, action, task_id, prompt_image, prompt_text: raw_prompt, duration, ratio } = JSON.parse(event.body);
    const prompt_text = (raw_prompt || '').replace(/^\*\*[^\n]+\*\*\n\n/, '').trim();
    ...
    // Runway only supports 5 or 10 seconds — map any value to nearest valid
    const parsedDur = parseInt(duration) || 10;
    const validDuration = parsedDur <= 7 ? 5 : 10;
