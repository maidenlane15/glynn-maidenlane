...
exports.handler = async function(event) {
  ...
  try {
    ...
    // Ratio map: Runway requires pixel format
    const ratioMap = {
    ...
    const mappedRatio = ratioMap[ratio] || '720:1280';
    const payload = {
      model: 'gen4_turbo',
      promptImage: prompt_image || undefined,
      promptText: prompt_text || '',
      duration: parseInt(duration) || 5,
      ratio: mappedRatio
    };
    // Remove undefined fields
    if (!payload.promptImage) delete payload.promptImage;

    const createResp = await fetch(BASE + '/v1/image_to_video', {
      method: 'POST',
      headers,
    ...
    if (createData.id) {
      ...
      return { statusCode: 200, body: JSON.stringify({ success: false, message: msg }) };
    }
